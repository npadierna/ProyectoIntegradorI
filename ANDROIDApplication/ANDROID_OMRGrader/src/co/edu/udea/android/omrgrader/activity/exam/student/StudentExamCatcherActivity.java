...
import co.edu.udea.android.omrgrader.imageprocess.model.GraderSession;
import co.edu.udea.android.omrgrader.imageprocess.model.ReferenceExam;
...
public class StudentExamCatcherActivity extends Activity {
	...
	private void createComponents() {
		...
		this.graderSession = new GraderSession(new ReferenceExam(null, super
				.getIntent().getStringExtra(REFERENCE_EXAM_KEY), null),
				new ArrayList<ReferenceExam>());
	}
	...
	private boolean requestIntentForTakingStudentPicture() {
		...
		try {
			takenReferencePictureFile = File.createTempFile(
					"TestingForDevices-",
					super.getResources().getString(R.string.pictures_prefix),
					directoryFile);

			this.graderSession.getStudentsOMRExamList().add(
					new ReferenceExam(null, takenReferencePictureFile
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case TAKE_STUDENT_PICTURE_REQUEST:
			if (resultCode == Activity.RESULT_OK) {
				...
				this.startReferenceExamProcessing("");
			}
			break;
		}
	}
	...
	private void startReferenceExamProcessing(String referenceExamRoutePath) {
		referenceExamRoutePath = "/storage/sdcard0/OMRGrader/tests/TestingForDevices_TEMP.JPG";
